...
func TestLoadFromExplicitPath(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte("ZERO_TEST_DOTENV_EXPLICIT=on\n"), 0o600); err != nil {
		t.Fatalf("write: %v", err)
	}
	t.Setenv("ZERO_DOTENV", path)
	os.Unsetenv("ZERO_TEST_DOTENV_EXPLICIT")
	Load()
	if got := os.Getenv("ZERO_TEST_DOTENV_EXPLICIT"); got != "on" {
		t.Fatalf("explicit ZERO_DOTENV path was ignored: %q", got)
	}
}
